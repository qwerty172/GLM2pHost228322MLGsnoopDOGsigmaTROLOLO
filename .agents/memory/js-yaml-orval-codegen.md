...
pnpm can hoist a transitive `js-yaml` dependency to a newer major (e.g. v5, ESM-only) that breaks orval, which expects the CJS v4 API. Symptom: codegen fails or produces garbage with no obvious error pointing at js-yaml.

**Why:** pnpm's hoisting picks the highest semver-compatible version across the tree unless pinned; orval's own dependency on js-yaml v4 doesn't stop a sibling package from pulling in v5.
**How to apply:** pin `js-yaml` via `pnpm.overrides` in the root `package.json` to a known-good v4.x, then reinstall and rerun codegen.
