  ...
  const saveVdsConfig = async (quotaId: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(formatApiErrorText(d.error, "Не удалось сохранить VDS-конфиг"));
      } else {
  ...
  const handleAiSuggest = async () => {
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        toast.error(formatApiErrorText(err.error, "AI вернул ошибку"));
        return;
      }
    ...
    try {
      ...
      if (publishNow) {
        try {
          ...
          toast.error(formatUserError(err, "Не удалось опубликовать"));
        }
      }
      navigate(`/quotas/${created.id}`);
    } catch (err) {
      toast.error(formatUserError(err, "Ошибка"));
    }
  };
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {vdsTestResult !== null && (
                        ...
                        >
                          ...
                          {vdsTestResult.ok
                            ? "Подключение успешно"
                            : formatApiErrorText(vdsTestResult.error, "Ошибка подключения")}
                        </span>
                      )}
                    </div>
                  </CardContent>
