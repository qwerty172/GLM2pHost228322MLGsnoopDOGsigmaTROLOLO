...
export default function Embed() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!cancelled) setError(extractEmbedError(err));
      }
    })();
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        if (valid.length > 0) iceServers = valid as RTCIceServer[];
      } catch {
  ...
  useEffect(() => {
    if (!liveSession || liveSession.status !== "ended" || ended) return;
    setEnded({ reason: liveSession.endReason ?? "ended" });
    cleanupConnection();
  }, [liveSession, ended, cleanupConnection]);
