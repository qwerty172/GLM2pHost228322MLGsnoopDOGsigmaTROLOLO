...
export default function Embed() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const result = await createEmbedSession({ apiKey, gameSlug, resolution, bitrateKbps });
        if (!cancelled) setSession(result);
      } catch (err) {
        if (cancelled) return;
        if (err instanceof ApiError && err.data && typeof err.data === "object" && "error" in err.data) {
          setError(err.data as EmbedApiError);
        } else {
          setError({
            error: "network_error",
  ...
  const playerTokenForPoll = session?.playerToken ?? "";
  const { data: polledSession } = useGetSessionByPlayerToken(playerTokenForPoll, {
    query: {
      enabled: !!playerTokenForPoll && !ended,
      queryKey: getGetSessionByPlayerTokenQueryKey(playerTokenForPoll),
