...
function readApiError(err: unknown): EmbedApiError {
  if (err && typeof err === "object" && "data" in err) {
    const data = (err as { data?: { error?: string; message?: string } | null }).data;
    if (data && typeof data === "object") {
      return {
        error: data.error ?? "unknown",
        message: data.message ?? "Не удалось начать сессию",
      };
    }
  }
  return {
...
export default function Embed() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const created = await createEmbedSession({ apiKey, gameSlug, resolution, bitrateKbps });
        if (!cancelled) setSession(created);
      } catch (err) {
        if (!cancelled) setError(readApiError(err));
      }
    })();
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    if (!polledSession || polledSession.status !== "ended") return;
