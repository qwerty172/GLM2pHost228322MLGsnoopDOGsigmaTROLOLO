...
export default function Embed() {
  ...
  const sessionStatusQuery = useGetSessionByPlayerToken(session?.playerToken ?? "", {
    query: {
  ...
  useEffect(() => {
    const s = sessionStatusQuery.data;
    if (!s || s.status !== "ended" || ended) return;
    setEnded({ reason: s.endReason ?? "ended" });
    cleanupConnection();
  }, [sessionStatusQuery.data, ended, cleanupConnection]);
