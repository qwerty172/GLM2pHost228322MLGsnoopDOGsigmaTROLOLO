...
function toEmbedError(err: unknown): EmbedApiError {
  if (err && typeof err === "object" && "data" in err) {
    ...
    if (data && typeof data === "object" && "error" in data) {
      ...
      return { error: body.error, message: body.message ?? body.error };
    }
  }
  return {
    error: "network_error",
...
export default function Embed() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!cancelled) setError(toEmbedError(err));
      }
    })();
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    if (!polledSession || polledSession.status !== "ended" || ended) return;
    setEnded({ reason: polledSession.endReason ?? "ended" });
    cleanupConnection();
  }, [polledSession, ended, cleanupConnection]);
