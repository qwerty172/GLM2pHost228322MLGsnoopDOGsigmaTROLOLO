...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    // Atomically claim the code so concurrent pair requests cannot both receive hostToken.
    ...
    const [host] = await db
      ...
      .from(hostsTable)
      .where(eq(hostsTable.id, claimed.hostId));
    ...
    req.log.info({ hostId: claimed.hostId }, "Agent paired via 6-digit code");
