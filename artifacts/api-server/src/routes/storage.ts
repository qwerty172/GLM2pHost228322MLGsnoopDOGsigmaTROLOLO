...
import { evaluateObjectReadAccess } from "../lib/objectAcl";
import {
  handleStorageError,
 ...
 * - no ACL metadata under uploads/* (legacy covers) → public READ
 * - no ACL metadata elsewhere → 401 (saves/clips must set ACL on upload)
 */
router.get("/storage/objects/*path", async (req: Request, res: Response) => {
  try {
    ...
    const access = await evaluateObjectReadAccess({
      userId,
      objectFile,
      entityId: wildcardPath,
    });
    if (!access.allowed) {
