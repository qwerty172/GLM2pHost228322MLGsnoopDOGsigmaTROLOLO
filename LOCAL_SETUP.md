...
С Docker Desktop: `pnpm setup` поднимает Postgres автоматически. Без Docker — создайте базу и пропишите `DATABASE_URL` в `.env` (см. `.env.example`).
...
Или: `chmod +x scripts/*.sh` и `./scripts/setup-local.sh` / `./scripts/dev-local.sh`.
