...
- [Docker](https://www.docker.com/) (рекомендуется) **или** [PostgreSQL 16](https://www.postgresql.org/download/windows/)

С Docker `pnpm setup` поднимет postgres и redis сам. Без Docker — создай базу:

```sql
CREATE DATABASE decentral_hub;
```

и укажи `DATABASE_URL` в `.env`.
