...
//   N. web components/*.tsx (non-ui) with export const data, no test (by file)
//
...
// --- J. host-agent main/*.ts without co-located test (by file) ------------
const mainDir = "artifacts/host-agent/src/main";
if (existsSync(mainDir)) {
  const mainModules = readdirSync(mainDir).filter(
  ...
  for (const mod of mainModules) {
    ...
    raw.push({
      cat: "J",
      groupKey: `j:${f}`,
      title: `host-agent main: unit-тест (${mod})`,
      file: f,
      detail: mod,
      items: [mod],
    });
...
// --- K. host-agent shared/*.ts without co-located test (by file) ----------
const sharedDir = "artifacts/host-agent/src/shared";
if (existsSync(sharedDir)) {
  const sharedModules = readdirSync(sharedDir).filter((f) => f.endsWith(".ts") && !f.endsWith(".d.ts"));
  for (const mod of sharedModules) {
    ...
    raw.push({
      cat: "K",
      groupKey: `k:${f}`,
      title: `host-agent shared: unit-тест (${mod})`,
      file: f,
      detail: mod,
      items: [mod],
    });
...
// --- N. web components/*.tsx (non-ui) with export const, no test (by file) --
const webCompDir = "artifacts/web/src/components";
if (existsSync(webCompDir)) {
  const compModules = readdirSync(webCompDir).filter((f) => f.endsWith(".tsx"));
  for (const mod of compModules) {
    ...
    if (testCandidates.some((t) => existsSync(t))) continue;
    const txt = readFileSync(`${webCompDir}/${mod}`, "utf8");
    if (!/\bexport const \w+/.test(txt)) continue;
    const f = `${webCompDir}/${mod}`;
    raw.push({
      cat: "N",
      groupKey: `n:${f}`,
