...
if (existsSync(webComponentsDir)) {
  ...
  for (const mod of componentModules) {
    ...
    const testCandidates = [
      `artifacts/web/test/${base}.test.mjs`,
      `artifacts/web/test/${base}.test.ts`,
      // PascalCase files map to kebab-case test names (KeyboardOverlay → keyboard-overlay)
      `artifacts/web/test/${base.replace(/([a-z])([A-Z])/g, "$1-$2").toLowerCase()}.test.mjs`,
      `artifacts/web/test/${base.replace(/([a-z])([A-Z])/g, "$1-$2").toLowerCase()}.test.ts`,
    ];
    if (testCandidates.some((t) => existsSync(t))) continue;
