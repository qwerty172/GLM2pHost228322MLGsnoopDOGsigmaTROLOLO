...
function formatFileCell(file) {
  if (file.includes("renderer")) return "`renderer/*.ts`";
  if (file.startsWith("artifacts/web/src/lib/")) return `\`${file.replace(/^artifacts\/web\/src\//, "")}\``;
  return `\`${shortRouteFile(file)}\``;
}

function formatRow(c, status = "pending") {
  const fileCell = formatFileCell(c.file);
