...
//   N. web components/*.{ts,tsx} without co-located test (non-ui/, by file)
//
...
// --- N. web components/*.{ts,tsx} without co-located test (non-ui/) -------
const webComponentsDir = "artifacts/web/src/components";
if (existsSync(webComponentsDir)) {
  const compModules = readdirSync(webComponentsDir).filter(
    (f) => (f.endsWith(".ts") || f.endsWith(".tsx")) && !f.endsWith(".d.ts"),
  );
  for (const mod of compModules) {
    ...
    if (testCandidates.some((t) => existsSync(t))) continue;
    const txt = readFileSync(`${webComponentsDir}/${mod}`, "utf8");
    if (!/\bexport (async )?function\b|\bexport const \w+/.test(txt)) continue;
