...
check_json() {
  ...
  local code
  local curl_args=(-s -o /tmp/agent-smoke-body.json -w "%{http_code}")
  ...
  if [[ "$code" == "$expected" ]] || [[ "$expected" == "2xx" && "$code" =~ ^2 ]]; then
    ...
    cat /tmp/agent-smoke-body.json >&2 || true
    return 1
  fi
}

json_field() {
  node -e "const d=JSON.parse(require('fs').readFileSync('/tmp/agent-smoke-body.json','utf8')); const v=process.argv[1].split('.').reduce((o,x)=>o?.[x],d); if(v==null) process.exit(1); console.log(v);" "$1"
}
...
node -e "const j=require('/tmp/agent-smoke-body.json'); if(!j.iceServers) process.exit(1);"
echo "OK  ice-config JSON valid"

check_json GET /api/downloads/host-agent.zip 2xx

ZIP_TYPE=$(file -b /tmp/agent-smoke-body.json 2>/dev/null || true)
# curl wrote body to file — verify zip magic
if ! head -c 2 /tmp/agent-smoke-body.json | grep -q PK; then
