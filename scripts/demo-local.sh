...
# Требует: запущенный API (./scripts/dev-local.sh) и PostgreSQL.
set -euo pipefail
...
if [[ ! -f .env ]]; then
  echo "Нет .env — сначала: ./scripts/setup-local.sh" >&2
  exit 1
fi
...
if [[ "$health" != *"ok"* ]]; then
  ...
  echo "Запусти: ./scripts/dev-local.sh" >&2
  exit 1
fi
echo "OK  healthz"
...
if [[ -z "$host_token" ]]; then
  # Rate limit — берём последний хост из БД
...
echo "  ${player_token}"
echo ""
echo "Подсказка: в development гости получают стартовый баланс автоматически."
