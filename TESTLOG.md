...
## C1-S06 — Storage ACL legacy public read (2026-08-02) {#marathon-c1-s06}

| Изменение | Детали |
|---|---|
| GET `/storage/objects/*` | Объекты без ACL metadata → 401/403 (legacy public read закрыт) |
| Saves | Private ACL (`player:{id}`) на `/saves/confirm` и `/players/me/saves/:gameId/commit` |
| Covers | Public ACL на `POST /games/submit` и `POST /admin/games/submissions/:id/approve` |
| Helper | `storageAcl.ts` — `normalizeObjectEntityPath`, `trySetStorageObjectAcl` |
| Tests | `objectAcl.test.ts` — 11 тестов (path normalize + canAccessObject) |
