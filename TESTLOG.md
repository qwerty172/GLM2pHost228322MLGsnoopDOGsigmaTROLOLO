...
| Проверка | Результат |
|---|---|
| use-player-wallet.tsx | экспорт PLAYER_WALLET_STORAGE_KEY, readStored*, registerGuestPlayerWallet, upgradeGuestPlayerWallet |
| use-player-wallet.test.mjs | 8 тестов — storage keys, guest register (existing/new/error), guest upgrade (guard/success/failure) |
| web test | PASS (71 tests) |
| typecheck | PASS |

**Следующий pick:** idle (Wave .tsx hooks закрыта).
