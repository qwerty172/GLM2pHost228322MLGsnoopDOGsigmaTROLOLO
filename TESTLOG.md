...
| **marathon** | **2026-07-29** | Wave UX: UX-02 troubleshoot + UX-03 TURN/STUN hints — см. MARATHON.md |
...
| Задача | Результат |
|---|---|
| UX-02 dashboard agent | Таблица симптомов, карточка stale heartbeat, авто-раскрытие «Расширенно» при ошибках агента |
| UX-03 TURN/STUN hints | Карточка «Сеть для игроков» на дашборде (STUN/TURN badges + подсказки); disconnect-hints в play без тех. терминов |

Верификация: `pnpm --filter @workspace/web typecheck`, api-server test (25), host-agent test (12).
