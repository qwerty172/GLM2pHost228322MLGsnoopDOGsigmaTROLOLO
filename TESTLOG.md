...
| Изменение | Детали |
|---|---|
| `GET /storage/objects/*` | Объекты без ACL metadata больше не читаются анонимно (401/403) |
| Saves | `POST /saves/confirm` и `POST /players/me/saves/:gameId/commit` ставят `visibility=private` |
| Covers | `POST /games/submit` ставит `visibility=public` для storage-обложек |
| Тесты | `objectAcl.test.ts` — 11 кейсов (canAccessObject + path helpers) |
| Верификация | `pnpm --filter @workspace/api-server test` — 33/33; host-agent 12/12 |
