...
| Изменение | Детали |
|---|---|
| `/storage/objects/*` | Объекты без ACL metadata больше не отдаются публично (401/403) |
| Covers | `visibility: public` ACL на submit и admin approve |
| Saves | `visibility: private` ACL на confirm/commit |
| Tests | `objectAcl.test.ts` — 16 кейсов (canAccessObject, parseStorageObjectPath, ensureObjectAclFromUrl) |
