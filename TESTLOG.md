...
## Marathon C1-S06 — Storage ACL legacy public read (2026-08-02) {#marathon-c1-s06}

| Находка | Действие |
|---|---|
| `GET /storage/objects/*` без ACL отдавал любой объект (в т.ч. saves) | Legacy public read ограничен путями `/objects/uploads/{id}` |
| Новые обложки без ACL metadata | `POST /storage/uploads/confirm` выставляет `visibility=public` + owner host |
| Unit | `objectAcl.test.ts` — path guard для legacy covers |
