...
| **marathon** | **2026-07-30** | UX-02 dashboard agent troubleshoot — см. MARATHON.md |
...
## Marathon UX-02 (2026-07-30) {#marathon-ux-02}

| Область | Изменение |
|---|---|
| Dashboard agent | `agent-status-stale` — карточка при устаревшем heartbeat |
| Dashboard agent | `agent-symptom-table` — таблица симптом → причина → действие |
| Dashboard agent | Контекстный troubleshoot-чеклист по состоянию агента |
| Dashboard agent | Авто-раскрытие «Расширенно» при ошибках агента / stale heartbeat |
| Dashboard agent | RU-перевод типовых сообщений событий агента (`formatAgentEventMessage`) |
| Verified | `pnpm --filter @workspace/web typecheck`; api-server 22+3; host-agent 12 |
