...
| Область | Фикс |
|---|---|
| `GET /storage/objects/*` | Объекты без ACL metadata больше не читаются публично (401/403) |
| Saves | `POST /saves/confirm` и `POST /players/me/saves/:gameId/commit` выставляют `visibility: private`, owner `player:{id}` |
| Covers | `POST /games/submit` и `POST /admin/games/submissions/:id/approve` выставляют `visibility: public` для object-storage обложек |
| Tests | `objectAcl.test.ts` — 11 vitest кейсов (canAccessObject + path helpers) |
