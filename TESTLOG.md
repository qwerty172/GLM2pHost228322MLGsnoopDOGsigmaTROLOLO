...
| Изменение | Детали |
|---|---|
| GET `/storage/objects/*` | Объекты без ACL → 403, кроме обложек из `games` / `game_submissions` |
| POST `/storage/uploads/confirm` | Хост подтверждает cover-upload → `visibility: public` ACL |
| OpenAPI | `confirmStorageUpload` + codegen hooks |
| Тесты | `storageObjectPath.test.ts` — нормализация путей |
