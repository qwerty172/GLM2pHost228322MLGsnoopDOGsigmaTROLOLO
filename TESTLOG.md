...
## Marathon C1-S06 (2026-08-02) {#marathon-c1-s06}

- **Storage ACL legacy public read closed:** `/storage/objects/*` всегда проверяет ACL; объекты без metadata → 401/403
- **ACL на write-path:** private на `games/submit` (cover), public на `admin/approve` и `admin/games PATCH`; private на `saves/confirm` и `saves/commit`
- **Тесты:** `objectAcl.test.ts` (9 кейсов canAccessObject + parseStorageObjectEntityPath)
