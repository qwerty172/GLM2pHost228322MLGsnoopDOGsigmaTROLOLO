...
## Marathon M-40 (2026-08-04 12:08 UTC) {#marathon-m-40}

| Проверка | Результат |
|---|---|
| game-detail.tsx | 6 raw fetch → `useSteamLookup`, `useListPublicGameHosts`, `createPreviewSession`, `getPublicIceConfig`, `publicPing`; локальный `useBrowserPingMs` → `@/hooks/use-browser-ping` |
| typecheck | PASS (web, game-detail без ошибок) |
