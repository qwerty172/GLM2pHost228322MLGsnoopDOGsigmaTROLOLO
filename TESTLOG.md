...
## Marathon M-41 (2026-08-04 13:10 UTC) {#marathon-m-41}

| Проверка | Результат |
|---|---|
| embed.tsx | 3 raw fetch → codegen (`createEmbedSession`, `getPublicIceConfig`, `useGetSessionByPlayerToken` + `refetchInterval: 5000`) |
| typecheck | PASS (monorepo) |
| marathon-scan | embed.tsx больше не в raw hits F |

**Следующий pick:** M-42 `web: raw fetch → codegen (1 call)`.
