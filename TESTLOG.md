...
| Saves | `confirm` / `commit` выставляют `visibility: private`, owner `player:{id}` |
| Covers | `games/submit` и admin `approve` выставляют `visibility: public` для storage paths |
| Tests | `objectAcl.test.ts` — 9 кейсов ACL + path helpers |
