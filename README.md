...
**Нет PostgreSQL?** Запусти БД в Docker:

```bash
pnpm setup:docker   # поднимает postgres+redis, настраивает DATABASE_URL
pnpm dev
```

Проверка: http://localhost:5000/games и http://localhost:8080/api/healthz
...
- Node.js 20+, pnpm 9+
- PostgreSQL 16 **или** Docker (для `pnpm setup:docker`)

### Запуск

```bash
pnpm dev            # один скрипт — API + Web
