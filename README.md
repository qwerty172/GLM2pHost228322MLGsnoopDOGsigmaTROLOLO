...
**На потом** (не нужны для первого запуска): TURN/coturn (WebRTC через NAT), Windows host-agent, Redis в проде, внешние API-ключи (VirusTotal, крипто-депозиты). См. [`infra/README.md`](./infra/README.md) и [`HOSTING.md`](./HOSTING.md).
