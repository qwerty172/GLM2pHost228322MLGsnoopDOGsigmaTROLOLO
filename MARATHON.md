...
> **Последнее обновление:** 2026-08-05 (M-81 done; KeyboardOverlay unit-тест + категория N сканера)
...
**Wave Maintenance:** M-81 done; M-82…M-89 pending (категория N — web components).
...
4. **Группировка:** C = по route-файлу; E/H = lib/renderer без тестов одной задачей; F = raw fetch по файлу; G = HOSTING backlog (H-NN); N = web components (без ui/); vendor `public/games/` и `isDev` console — исключены.
5. Приоритет категорий: B TODO → C OpenAPI → A RU → G HOSTING → F fetch → E renderer-тесты → H api-lib → J/K main/shared → L/M/N web lib/hooks/components → D debug → I eslint.
