...
> **Последнее обновление:** 2026-08-05 (M-241 done; walletAddresses export coverage)

## Last run (automation)

| Поле | Значение |
|------|----------|
| Дата | 2026-08-05 23:49 UTC|
| Task ID | M-241|
| Результат | walletAddresses.test.ts: generateNanoAddress, generateTronUsdtAddress, generateAllDepositAddresses (4 tests PASS)|
...
| scanner_empty_idle | `exit 3` — **агент не поднимается**. Пустая очередь это норма, а не повод искать себе работу. Пополнение бэклога — решение человека: `planned` → `todo` в [UX_BACKLOG.md](./UX_BACKLOG.md) + `--sync-marathon`. |
| pr_in_flight | STOP или закрыть мёртвый PR >1ч |
| полный idle | `exit 3` — cron не поднимает агента вообще (экономия токенов) |

**Расширение сканера** больше не происходит само: нужен явный `MARATHON_ALLOW_EXPAND=1`.
Именно авто-expand каждую минуту давал петлю на 1000+ пустых runs.

### Гейт качества (обязателен перед `done`)

```bash
node scripts/marathon-verify.mjs --task M-NN
```

Блокирует три способа «закрыть» задачу без работы: пустой `git diff`,
коммит только в `MARATHON.md`/`TESTLOG.md`, красные тесты затронутых пакетов.
`exit 1` → статус `done` ставить нельзя.
...
> **Запускает человек, не automation.** Ночной прогон не может проверить UI глазами,
> поэтому строки волны U-2 лежат со статусом `planned` и в очередь не попадают.
> Чтобы запустить UX-волну: перевести нужные строки `planned` → `todo`, затем
...
| M-187 | U | db schema: unit-тест (devKeys.ts) | `db/devKeys.ts` | u:lib/db/src/schema/devKeys.ts | done | agent |
| M-188 | U | db schema: unit-тест (gameSubmissions.ts) | `db/gameSubmissions.ts` | u:lib/db/src/schema/gameSubmissions.ts | done | agent |
| M-189 | U | db schema: unit-тест (games.ts) | `db/games.ts` | u:lib/db/src/schema/games.ts | done | agent |
| M-190 | U | db schema: unit-тест (hostGames.ts) | `db/hostGames.ts` | u:lib/db/src/schema/hostGames.ts | done | agent |
...
| M-197 | U | db schema: unit-тест (playerGameSaves.ts) | `db/playerGameSaves.ts` | u:lib/db/src/schema/playerGameSaves.ts | done | agent |
| M-198 | U | db schema: unit-тест (players.ts) | `db/players.ts` | u:lib/db/src/schema/players.ts | done | agent |
