...
| Поле | Значение |
|------|----------|
| Дата | 2026-08-05 13:58 UTC|
| Task ID | M-147, M-148|
| Результат | U-31: `/downloads/host-agent.exe` авто-резолвит GitHub Release вместо 503; честная `.exe`-ссылка на дашборде (не primary — installer не несёт per-host токен). U-32: unzip-тест `hostToken` в config.json (`adm-zip`). api-server 15 PASS, web 327 PASS|
...
| M-149 | R | UX U-13 (P0): Дашборд всегда показывает одно следующее действие до первого | `pages/host/dashboard.tsx` | r:U-13 | pending | agent |
