...
/**
 * Register a getter that supplies the host token for admin routes that require
 * the `X-Host-Token` header (in addition to or instead of Bearer auth).
 */
...
/**
 * Register a getter that supplies the deployment admin secret for routes that
 * require the `X-Admin-Secret` header.
 */
