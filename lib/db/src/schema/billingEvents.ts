...
export const billingEventsTable = pgTable("billing_events", {
  ...
  hostIdx: index("billing_events_host_idx").on(t.hostId),
  // Hot path: per-session tick history for billing audits / refunds.
  sessionTickIdx: index("billing_events_session_tick_idx")
    .on(t.sessionId, t.billedAt)
    .where(sql`${t.kind} = 'session_tick'`),
}));
